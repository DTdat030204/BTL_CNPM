        ...
        useEffect(() => {
            const fetchInfo = async () => {
                try {
                    ...
                    setPrintingLog(response.data.printingLogs || []); // Update state with the printing logs
                } catch (error) {
            ...
            <Box sx={{ width: '80%', mx: 'auto', mt: '5vh', p: 2, backgroundColor: '#F5F5F5', borderRadius: '10px' }}>
                ...
                <TableContainer component={Paper}>
                    <Table>
                        <TableHead>
                            <TableRow>
                                <TableCell align="center"><strong>Tên Tài Liệu</strong></TableCell>
                                <TableCell align="center"><strong>Kích thước tài liệu</strong></TableCell>
                                <TableCell align="center"><strong>Khổ giấy</strong></TableCell>
                        ...
                        <TableBody>
                            {printingLog.map((log) => (
                                <TableRow key={log.id}>
                                    ...
                                    <TableCell align="center">{log.size + ' MB'}</TableCell>
                                    <TableCell align="center">{log.pageSize}</TableCell>
                                    <TableCell align="center">{log.numOfPages}</TableCell>
                                    <TableCell align="center">{log.numOfCopies}</TableCell>
                                    <TableCell align="center">{new Date(log.startDate).toLocaleString()}</TableCell>
                                    <TableCell align="center">{new Date(log.endDate).toLocaleString()}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
